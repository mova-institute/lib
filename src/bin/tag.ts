...
import {
  tokenizeMixml,
  morphInterpret,
  numerateTokensGently,
  mixml2tokenStream,
  string2tokenStream,
  tokenStream2plainVertical,
  tokenizeUk,
  autofixDirtyText,
  normalizeCorpusText,
  newline2paragraph,
...
function main(args: Args) {
  ...
  ioArgsPlain(async (input, outputFromIoargs) => {
    const analyzer = createAnalyzer(args).setExpandAdjectivesAsNouns(
      args.expandAdjAsNoun,
    )
    ...
    if (args.format === 'xml') {
      ...
      if (
        !/^<[^>]*xmlns:mi="http:\/\/mova\.institute\/ns\/corpora\/0\.1"/.test(
          inputStr,
        )
      ) {
      ...
      if (args.reinterpret) {
        morphReinterpret(
          root.evaluateElements('//mi:w_|w_', NS).toArray(),
          analyzer,
        )
      }
      ...
      if (args.forMorphDisamb) {
        root
          .evaluateElements('//mi:w_|//w_', NS)
          .map((x) => $t(x))
          .filter((x) =>
            x.getDefiniteInterps().some((x) => x.flags.startsWith('punct')),
          )
      ...
      if (args.unknown) {
        ...
        let unknowns = [
          ...new Set(
            root
              .evaluateElements('//mi:w_[w[@ana="x"]]|w_[w[@ana="x"]]', NS)
              .map((x) => $t(x).text()),
          ),
        ]
        if (args.sort) {
      ...
      if (args.count) {
        ...
        let tokens = string2tokenStream(inputStr, analyzer)
          .filter(
            (x) => x.isWord() && x.interp0().isX() && !x.interp0().isForeign(),
          )
          .map((x) => x.form)
          .unique()
          .toArray()
        ...
        if (args.reinterpret) {
          tokenStream = tokenStream.window(2).map(([curr, next]) => {
            if (curr.form) {
              let newInterps = analyzer
                .tag(curr.form, next && next.form)
                .filter(
                  (x) => !curr.interps.find((xx) => xx.featurewiseEquals(x)),
                )
        ...
        if (args.format === 'cg') {
          ...
          tokenStream2plainVertical(tokenStream, args.mte).forEach((x) =>
            output.write(x + '\n'),
          )
        }
      }
