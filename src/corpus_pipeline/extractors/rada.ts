...
export function extract(fileContents: string): CorpusDoc {
  ...
  */
  ...
  let [headerPars, contentPars] = clusterize(htmlLines,
    line => lineClassifiers.findIndex(x => x(line)))

  if (!contentPars) {  // wierd format e.g. 2/19960627-1
    console.error(`skipping`)
    return
  }

  // hm…
  [headerPars, contentPars].forEach(x => x && mapInplace(x, [
    removeTags,
    he.unescape,
    normalizeZvidusilParaNondestructive
  ]))

  contentPars = contentPars.filter(x => x
  ...
  for (let p of contentPars) {
    ...
    // console.error(p)
    let match = p.match(/^((?:ГОЛОВА|ГОЛОВУЮЧИЙ|Головуючий|ГОЛОСИ? ІЗ ЗАЛУ)\.|[А-ЯІЇЄҐ]{2,} [А-ЯІЇЄҐ]\.[А-ЯІЇЄҐ]\.) (.*)/)
    if (match) {
