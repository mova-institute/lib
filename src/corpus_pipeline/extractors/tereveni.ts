...
export function* streamDocs(html: string) {
  ...
  let posts = root
    .evaluateElements('//div[contains(@class, "post_wrap")]')
    .toArray()
  // console.log(`---${posts.length}----`)
  for (let postRoot of posts) {
    let title = textOf(
      postRoot,
      './/span[contains(@class, "post_id ")]/a/text()',
    ).trim()
    ...
    let postEntry = postRoot.evaluateElement(
      './/div[contains(@class, "post entry-content")]',
    )
...
function getPostParagraphs(contentRoot: AbstractElement) {
  ...
  traverseDepth(contentRoot, (node) => {
    if (node.isElement()) {
      ...
      if (
        ['citation', 'blockquote', 'edit'].includes(cssClass) ||
      ...
      if (el.localName() === 'br') {
        ...
        el.localName() === 'img' &&
        el.attribute('class') === 'bbc_emoticon'
      ) {
