...
export function testMte2Vesum(fileStr: string) {
  ...
  for (let {form, tag, lemma, lemmaTag, lineNum} of iterateDictCorpVizLines(lines)) {
    ...
    try {
      var mte1 = rysin2multext(lemma, lemmaTag, form, tag)[0];
      var fromMte = MorphTag.fromMte(mte1);
      var vesum2 = fromMte.toVesumStr();
      var mte2 = rysin2multext(lemma, lemmaTag, form, vesum2)[0];
      if (mte1 !== mte2) {
