...
export function writeCompiledDict(path: string, compiledDict) {
  // todo: type
  writeFileSync(path + '/tags.json', JSON.stringify(compiledDict.tags), 'utf8')
  writeFileSync(
    path + '/suffixes.json',
    JSON.stringify(compiledDict.suffixes),
    'utf8',
  )
  writeFileSync(
    path + '/words.dawg.json',
    JSON.stringify(compiledDict.words),
    'utf8',
  )
  ...
  for (let paradigm of compiledDict.paradigms) {
    ...
    writeSync(
      f,
      new Buffer(new Uint8Array(paradigm.buffer)),
      0,
      paradigm.byteLength,
    )
  }
