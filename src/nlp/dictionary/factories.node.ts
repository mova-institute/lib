...
export function createDictionarySync(
  dictFolder = getLibRootRelative('../data/dict/vesum'),
) {
  ...
  while (curByte < buf.length) {
    let paradigmByteLen =
      buf.readUInt16LE(curByte) * 3 * Uint16Array.BYTES_PER_ELEMENT
    curByte += Uint16Array.BYTES_PER_ELEMENT
    paradigms.push(
      new Uint16Array(
        buf.buffer,
        curByte,
        paradigmByteLen / Uint16Array.BYTES_PER_ELEMENT,
      ),
    )
    curByte += paradigmByteLen
  }

  let words = readStringMapDawgSync<WordDawgPayload>(
    dictFolder + '/words.dawg',
    WordDawgPayload.create,
  )
