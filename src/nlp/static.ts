...
export const INVISIBLES_RE =
  /[\u0000-\u0008\u000E-\u001F\u007F-\u009F\u200B-\u200F\u202A-\u202E\u2060]/gu
export const WCHAR_UK = r`\-’${LETTER_UK}`
export const WCHAR_UK_UPPERCASE = r`\-’${LETTER_UK_UPPERCASE}`
export const FOREIGN_RE = new RegExp(
  `^[${WCHAR_UK}]*[A-Za-zЫыЁёЪъЭэ]+[${WCHAR_UK}]*$`,
) // not negation
...
export const NUMERAL_PREFIXED_TOKEN_RE = new RegExp(
  r`^(\d+)-([${APOSTROPES}${LETTER_UK}]+)$`,
)
...
export const PUNC_GLUED_AFTER = Object.keys(PUNC_SPACING)
  .filter((x) => PUNC_SPACING[x][0])
  .map((x) => '\\' + x)
  .join('')
export const PUNC_GLUED_BEFORE = Object.keys(PUNC_SPACING)
  .filter((x) => PUNC_SPACING[x][1])
  .map((x) => '\\' + x)
  .join('')
export const NO_GLUE_PUNC = Object.keys(PUNC_SPACING)
  .filter((x) => PUNC_SPACING[x][0] && PUNC_SPACING[x][1])
  .map((x) => '\\' + x)
  .join('')

export const SMILIES_1 = [r`О_о`, r`0_0`].join('|')
