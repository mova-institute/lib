...
export function* expandDictCorpViz(lines: Iterable<string>) {
  ...
  lexemeLoop: for (let lexeme of chunkLexemes(lines)) {
...
export function* domesticateDictCorpViz(fileStr: string) {
  ...
  lexeme: for (let lexeme of expandDictCorpViz(lines)) {
    let mustDropNumber =
      /\snumr:/.test(lexeme[0]) &&
      lexeme.some((x) => /\s.*\b:p\b/.test(x)) &&
      !lexeme.some((x) => !/\s.*\b:p\b/.test(x))

    for (let { form, tag, lemma, lemmaTag, isLemma } of iterateDictCorpVizLines(
      lexeme,
    )) {
...
export function presentTagsForDisamb(interps: Array<IStringMorphInterp>) {
  ...
  interps.forEach((x) =>
    (isAdditionalTag(x.flags) ? auxTags : mainTags).push(x),
  )
  return {
...
export function presentTagsForDisambOneBlock(
  interps: Array<IStringMorphInterp>,
) {
  let splitted = interps.map((x, index) => ({
    index,
    lemma: x.lemma,
    flags: x.flags.split(':'),
  }))
  let sorted = stableSort(splitted, (a, b) =>
    compareTags(MorphInterp.fromVesum(a.flags), MorphInterp.fromVesum(b.flags)),
  )

  let aligned = alignTagList(sorted.map((x) => x.flags))
  let flags = aligned.map((x) =>
    x.map((xx) => new Array<{ content: string; isMarked: boolean }>()),
  )
...
function alignTagList(flags: Array<Array<string>>) {
  ...
  for (let posAgg of poses.values()) {
    ...
    pos = pos ? pos.vesum : ''
    let featureOrderMap = arr2indexMap(
      (FEATURE_ORDER[pos] || FEATURE_ORDER.other).filter((x) =>
        features.has(x),
      ),
    )
...
export function findUnidentifiableRows(fileStr: string) {
  ...
  for (let { form, tag, lemma } of iterateDictCorpVizLines(
    fileStr.split('\n'),
  )) {
...
export function* gatherXps(lines: Iterable<string>) {
  lines = mu(lines)
    ...
    .sort(
      (a: string, b: string) =>
        collator.compare(a.match(/^\S+/)[0], b.match(/^\S+/)[0]) ||
        xpNumber(a) - xpNumber(b),
    )
