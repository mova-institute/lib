const tagMap = {
  ...
  'v_dav': { feat: 'case', mte: 'd' },
  'v_zna': { feat: 'case', mte: 'a' },
  ...
  constructor(flags: string) {
    for (let flag of flags.split(':')) {
      if (flag in tagMap) {
        ...
        this.pushToArrayFeature('altPoses', 'adjp');
      }
      else if (flag.startsWith('&_')) {
        this.pushToArrayFeature('altPoses', flag.substr(2));
      }
      else if (flag.startsWith('&')) {
        this.shadowPos = this.pos;
        this.pos = flag.substr(1);
      }
    }
  }
...
export function rysin2multext(lemma: string, lemmaTagStr: string, form: string, formTagStr: string) {
  ...
  if (!treatSpecialCases(ret, form, formTag)) {
    for (let pos of formTag.poses()) {
      ...
      switch (formTag.pos) {
        ...
        case 'pron': {
          ...
          let syntacticType = mapTag(formTag.shadowPos).toLowerCase();
