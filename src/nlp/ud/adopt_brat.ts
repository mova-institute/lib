...
function main() {
  ...
  let allSyntBratFiles = allBratFiles.filter((x) => x.includes('/treebank/'))
  let allCorefBratFiles = allBratFiles.filter((x) => x.includes('/coref/'))
  ...
  for (let bratFile of allSyntBratFiles) {
    for (let span of parseBratFile(linesSync(bratFile))) {
      ...
      if (!el) {
        // when tokens are deleted in xml but remain in brat
        continue
      }
